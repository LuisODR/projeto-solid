...
class Conta(ABC):
    ...
    def depositar(self, valor: float):
        ...
        self.saldo += valor
        self.transacoes.append(Transacao(len(self.transacoes)+1, datetime.now(), valor, "Depósito"))
    ...
    def sacar(self, valor: float):
        ...
        self.saldo -= valor
        self.transacoes.append(Transacao(len(self.transacoes)+1, datetime.now(), -valor, "Saque"))

    def transferir(self, destino: "Conta", valor: float):
        self.sacar(valor)
        destino.depositar(valor)
        self.transacoes.append(Transacao(len(self.transacoes)+1, datetime.now(), valor, f"Transferência para {destino.numero}"))
...
if __name__ == "__main__":
    ...
    for t in banco.contas[0].transacoes:
        print(t.registrar())
